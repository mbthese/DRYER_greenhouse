...
file_samples <- read.delim2("E:/DRYER_greenhouse_data/DRYER_serre_ITS_ngsfilter.tab", header = FALSE)
...
#obifile
file_obitab <- read.delim2("E:/DRYER_greenhouse_data/Fasteris_ITS/Post-pipeline_obitools/AMVG42-230512/DRYER_serre_R1R2_good_demultiplexed_derepl_basicfilt_cl_agg.tab")
...
#read
colnames(file_obitab)
read <- file_obitab %>% dplyr::select(c(1,20:339))
...
#motus
motus <- file_obitab[, c(1,19,359)] #take the sequence column
...
samples_to_remove <- c("DRYER_greenhouse_Leaf_ITS__891", "DRYER_greenhouse_Leaf_ITS__PCR-tags_29",
"DRYER_greenhouse_Leaf_ITS__526","DRYER_greenhouse_Leaf_ITS__741", "DRYER_greenhouse_Leaf_ITS__366", "DRYER_greenhouse_Leaf_ITS__458", "DRYER_greenhouse_Leaf_ITS__941")      
...
save(MetabarList, file = "./results/Metabarlist_serre_ITS_AMVG42-230512.Rdata")

#assign 
assign <- read.csv("E:/DRYER_greenhouse_data/Fasteris_ITS/taxa.serre.AMVG42-230512.ITS.csv", header = TRUE)
...
sum(MetabarList$pcrs$nb_reads) # 1131630 de reads
Nbr_reads <- MetabarList$pcrs

#total number of reads
sum(Nbr_reads$nb_reads) #[1] 1131630
...
ggarrange(A, B, labels = c("Fungi (AMVG42-0512) - nr_reads for all with controls" ,"nb_reads for samples"))

# Compute the number of motus per pcr
MetabarList$pcrs$nb_motus <- rowSums(MetabarList$reads>0)
summary(MetabarList$pcrs$nb_motus <10) #only 24 beneath 10.
#average: 403 per samples

length(MetabarList$motus$sequence)  #6495
length(unique(MetabarList$motus$sequence))
#4790 #nombre d'OTUs unique.
...
pcrs     1131630     4790  3536.344 3869.888  124.8219 81.00093
samples  1128030     4767  4116.898 3891.697  143.3942 72.40736
...
gghill_rarefaction(natura.raref) + ggtitle("AMVG42-0512")
...
# $dataset_statistics
#         nb_reads nb_motus avg_reads sd_reads avg_motus sd_motus
# pcrs     1090425     4657  4782.566 3910.482  139.8026 72.41304
# samples  1090425     4657  4782.566 3910.482  139.8026 72.41304
...
```{r}
AMVG42_230512_ITS_clean <- serre_clean
```
...
AMVG42_230512_ITS_clean$samples <- left_join(AMVG42_230512_ITS_clean$samples, plant_info, by= "code")
```


##save
```{r}
save(AMVG42_230512_ITS_clean, file = "./results/Metabarlist_serre_ITS_AMVG42-230512_clean.Rdata")
```
...
```{r}
#LEAF 
load("./results/Metabarlist_serre_ITS_AMVG42-230512_clean.Rdata")
rownames(AMVG42_230512_ITS_clean$samples) <- AMVG42_230512_ITS_clean$samples$sample_id
summary_metabarlist(AMVG42_230512_ITS_clean)


AMVG42_230512_ITS_clean$samples$nb_reads <- rowSums(AMVG42_230512_ITS_clean$reads) #get number of reads per sample
AMVG42_230512_ITS_clean$samples$nb_motus <- rowSums(AMVG42_230512_ITS_clean$reads>0) #get number of OTUs per sample

#to get number of motus and reads
leaf2 <- melt(AMVG42_230512_ITS_clean$samples,id = c( "code", "Species", "Time", "Treatment", "nb_reads", "nb_motus"))
...
write.csv(table_leaf,"./results/summary_ITS2_AMVG42-230512.csv")

library(kableExtra)
table_leaf %>%
  dplyr::select(-organ) %>% #remove it otherwise redundant
  kbl(caption = "Curated data for AMVG42-230512-ITS2 : number of replicates, mean number of reads and OTUs per species.", escape = FALSE) %>% #title
  row_spec(0, bold = TRUE) %>% #first row is in bold
  column_spec(1, italic = T) %>% #make species name in italic
  kable_classic(full_width = F, html_font = "Cambria") %>%
  save_kable(file = "./results/summary_ITS2_AMVG42-230512.png") #make it bigger add zoom = 5 in save_kable
