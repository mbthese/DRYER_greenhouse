...
#obifile
file_obitab <- read.delim2("E:/DRYER_greenhouse_data/Fasteris_ITS/Post-pipeline_obitools/AMVG52/DRYER_serre_R1R2_good_demultiplexed_derepl_basicfilt_cl_agg.tab")
...
#read
colnames(file_obitab)
read <- file_obitab %>% dplyr::select(c(1,20:344))
colnames(read)
...
#motus
motus <- file_obitab[, c(1,19,365)] #take the sequence column
rownames(motus) <- motus$id

#first construct the metabarlist without assignation
#assign 
assign <- read.csv("E:/DRYER_greenhouse_data/Fasteris_ITS/taxa.serre.AMVG52.ITS.csv", header = TRUE)
...
summary(file_obitab$seq_length) #265.2 (range 80-461)

#dimensions
dim(read_t) #325 3747
dim(motus) #3747 3
dim(assign_motus) #3747 10
dim(pcrs) #327 11
dim(samples)#327 11
...
samples_to_remove <- c("DRYER_greenhouse_Leaf_ITS__77","DRYER_greenhouse_Leaf_ITS__460" )      
...
#save(MetabarList, file = "./results/Metabarlist_serre_ITS_AMVG52.Rdata")
```
...
pcrs      513632     3747  1580.406 1450.348  102.7877
samples   511535     3737  1840.054 1411.498  117.4676
        sd_motus
pcrs    63.03821
samples 56.08340
...
gghill_rarefaction(natura.raref) + ggtitle("AMVG-52")
...
Since the dataset was obtained with primers that target ITS2, other MOTUs should be excluded. At this stage of the analysis, we only flag MOTUs based on this criterion.
...
table(serre$motus$target_taxon,
      serre$motus$not_a_sterilisation_conta) #4 MOTUs as non-taget, which were already flagged as potential sterilisation conta

table(serre$motus$target_taxon,
      serre$motus$not_an_extraction_conta) #13 MOTUs as non-taget, which were already flagged as potential ext conta

table(serre$motus$target_taxon,
      serre$motus$not_a_pcr_conta) #13 MOTUs as non-taget, which were already flagged as potential pcr conta
```
...
# pcrs      466070     3502  2402.423 1313.486  122.1856  56.8542
# samples   466070     3502  2402.423 1313.486  122.1856  56.8542
...
# $dataset_statistics
#         nb_reads nb_motus avg_reads sd_reads avg_motus sd_motus
# pcrs      466051     3494  2402.325 1313.452  122.1082 56.78916
# samples   466051     3494  2402.325 1313.452  122.1082 56.78916
...
AMVG52_ITS_clean <- serre_clean
```

# Add information on the plants
```{r}
plant_info <- read_csv("E:/DRYER_greenhouse_data/Individus_plantes_ITS_ALL.csv")
names(plant_info)

plant_info <- plant_info %>%
  rename(code = Code)

AMVG52_ITS_clean$samples <- left_join(AMVG52_ITS_clean$samples, plant_info, by= "code")
```

##save
```{r}
save(AMVG52_ITS_clean, file = "./resources/Metabarlist_serre_ITS_AMVG52.Rdata")
```
...
```{r}
#LEAF ITS - AMVG 52
load("./results/Metabarlist_serre_ITS_AMVG52.Rdata")
summary_metabarlist(AMVG52_ITS_clean)

AMVG52_ITS_clean$samples$nb_reads <- rowSums(AMVG52_ITS_clean$reads) #get number of reads per sample
AMVG52_ITS_clean$samples$nb_motus <- rowSums(AMVG52_ITS_clean$reads>0) #get number of OTUs per sample

#to get number of motus and reads
leaf2 <- melt(AMVG52_ITS_clean$samples,id = c( "code", "Species", "Time", "Treatment", "nb_reads", "nb_motus"))
...
write.csv(table_leaf,"./results/summary_ITS2_AMVG52.csv")

library(kableExtra)
table_leaf %>%
  dplyr::select(-organ) %>% #remove it otherwise redundant
  kbl(caption = "Curated data for AMVG52-ITS2 : number of replicates, mean number of reads and OTUs per species.", escape = FALSE) %>% #title
  row_spec(0, bold = TRUE) %>% #first row is in bold
  column_spec(1, italic = T) %>% #make species name in italic
  kable_classic(full_width = F, html_font = "Cambria") %>%
  save_kable(file = "./results/summary_ITS2_AMVG52.png") #make it bigger add zoom = 5 in save_kable
